...
> - **Traefik over NGINX**: Traefik has native Kubernetes CRD support (`IngressRoute`), a built-in dashboard, and excellent integration with cert-manager. It handles TLS termination using certificates managed by cert-manager. Traefik also resolves `ExternalName` service addresses directly from the Kubernetes API rather than via CoreDNS — this means `ExternalName` services can use raw IP addresses, which makes proxying to non-cluster infrastructure clean and ArgoCD-friendly. Other ingress controllers (NGINX, HAProxy) resolve ExternalName through CoreDNS, which rejects raw IPs. See `apps/traefik/external/README.md`.
